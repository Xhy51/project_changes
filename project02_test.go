...
func TestSearch(t *testing.T) {
	...
	idx, err := BuildIndexFromURLList(docs)
	if err != nil {
		t.Fatalf("BuildIndexFromURLList: %v", err)
	}

	// Query: "romeo" should surface pages from "Romeo and Juliet".
	hits := idx.SearchTFIDF("romeo")
	if len(hits) == 0 {
...
func TestTfIdf(t *testing.T) {
	...
	h2 := idx.SearchTFIDF("frankenstein")
	if len(h2) == 0 {
